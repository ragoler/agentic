...
class SummaryAgent:
    ...
    async def generate_summary(self, origin: str, destination: str, flights: List[LiveFlightState], hotels: List[HotelOption]) -> str:
        """
        Generates a natural language summary by creating a prompt and calling the MCP.
        """
        prompt = self._create_prompt(origin, destination, flights, hotels)
        
        # Delegate the external call to the MCP
        return await self.mcp.generate_text_summary(prompt)

    def _create_prompt(self, origin: str, destination: str, flights: List[dict], hotels: List[dict]) -> str:
        ...
        Generate a brief, friendly, and inviting summary for a trip from {origin} to {destination}.
